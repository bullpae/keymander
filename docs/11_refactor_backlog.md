# 리팩토링·개선 백로그

> **상태: 진행 중인 작업 목록** — 2026-08-08 전수 감사(보안·성능·확장성·코드 품질)
> 결과에 2026-08-27 재점검(자체 + Codex 교차 리뷰)을 통합한 것. 완료 항목은
> ✅로 남겨 "왜 이 코드가 이런 모양인지"의 근거를 보존한다.
> 우선순위: 깨진 것 → 근본/작은 것 → 매일의 불편 → 확장.
>
> **줄 수·테스트 수 같은 수치는 이 문서에 적지 않는다.** 손으로 적으면 며칠 만에
> 낡는다 — 실제로 `app.rs`가 2449줄로 적혀 있는 동안 2644줄이 됐다.
> 필요할 때 `./scripts/code-metrics.sh` 로 현재 값을 본다.

## 0. 이번 감사에서 확인된 강점 (건드리지 말 것)

- IPC 인증: CSPRNG 토큰 + 상수시간 비교 + 64KB 상한 + 타임아웃. 토큰 파일 0600 생성.
- 명령 실행: `Command::new + args` — shell 경유 없음, 인젝션 표면 없음.
- 키 입력·검색어를 로그에 남기는 코드 없음 (키로거화 방지).
- 검색: Nucleo + 사전 소문자화 — 이미 성능 지향.
- 자체 코드에 TODO/FIXME 마커 0건 (전부 vendor/).
- 테스트: kmd-core 171 · kmd-daemon 71 · kmd-desktop 77.

## 1. 신뢰성·안전 ✅ 대부분 완료

| 항목 | 상태 |
|---|---|
| FFI 훅 콜백 패닉 격리 (catch_unwind, 양 플랫폼) | ✅ 1fdf627 |
| 탭 타임아웃 서킷 브레이커 (CPU 기아 시 키보드 잠금 방지) | ✅ 2026-08-08 사고 후 |
| 인덱스 캐시 0600 + 데이터 디렉터리 0700 | ✅ 223ed85 |
| status의 키 훅 진단 (조용한 훅 사망 가시화) | ✅ |
| macOS 서명·TCC: 전용 키체인 + launchd 재시작 | ✅ docs/07 §7 |
| **R1-1. Windows alt+space 무반응** | ✅ 원인 규명·수정 (2026-08-07, Windows 머신에서 직접) — 데몬은 살아 있는데 **OS가 LL 훅을 조용히 제거**(Modern Standby 복귀·콜백 타임아웃)해 키맵만 죽는 문제였다. 실측: 데몬 22시간 가동 중 Standby 복귀 20회+, 훅 로그 전무. 하트비트+워치독+재설치 3단으로 자동 복구 (25bf21c, main 병합됨). status에 훅 생존 표시 추가. 실사용 검증 대기 |
| **R1-2. spawn+토글 레이스 제거** | ✅ 2026-08-08 — 창 표시 마커(kmd.shown) 도입: 창이 실제로 뜬 인스턴스만 토글 대상. 부팅 중 재-핫키는 조용히 물러나고, 10s 유예를 넘긴 미표시 인스턴스는 멈춘 것으로 보고 회수 허용. 3개 백엔드 공통, E2E(표시→마커→토글→정리) 검증 |
| **R1-3. 콜드 스타트 실측·개선** | 🔶 웜 스타트 실측 706ms(spawn→창 표시, 2026-08-08). 진짜 콜드(재부팅 직후/하루 첫 실행)는 사용자 환경에서만 측정 가능 — 다음 아침 첫 alt+space가 느리면 그 직후 `kmd-data/desktop/logs/desktop.log`의 부팅 타이밍 로그 확인. R1-2 덕에 느려도 재-핫키가 창을 죽이지는 않음 |
| **R1-4. 데몬 .app 번들화 검토** | ⬜ TCC가 번들을 훨씬 안정적으로 다룸 — 접근성 재부여 마찰의 근본 해법 후보 |

## 2. 코드 품질 (2026-08-08 스윕에서 발견)

| 항목 | 내용 |
|---|---|
| **R2-1. clippy 정리** | ✅ 유지 중 — 경고 0건. 주의: `VKey::ALL`·`DelayedRefocus`는 죽은 코드가 아니라 **cfg(windows) 전용 코드의 사용처**였다(macOS 빌드에서만 미사용으로 보임) — 삭제 대신 `cfg_attr(not(windows), allow(dead_code))` 처리. 삭제했으면 Windows가 깨졌다 |
| **R2-2. 대형 파일** | ✅ 주요 대상 완료. **줄 수는 여기 적지 않는다** — 손으로 적으면 며칠 만에 낡는다. 현재 값은 `./scripts/code-metrics.sh`.<br>· `keybind/engine.rs`의 `process_key` 451줄 → 진입점 43줄 + 12스텝 (2026-08-27). `process_key_동작_스냅샷` characterization 테스트가 의미 보존을 지킨다<br>· `kmd-desktop/app.rs`의 `update_inner` 359줄 → **84줄** (2026-08-28). 인라인 arm 11개를 파일의 기존 관례(`Message::X => self.handle_x()`)대로 핸들러 메서드로 추출. 이제 이 파일에 60줄 넘는 함수가 없다<br>· 남은 것: `app/view.rs`의 `view`(223줄)·`view_detail_panel`(231줄). iced 레이아웃 빌더라 분해 이득이 작아 **보류** — 조건부 표시가 늘어 읽기 어려워지면 그때 |
| **R2-3. CLI/TUI 테스트 공백** | ✅ 대부분 해결 (2026-08-28) — `src/` 테스트 4개 → **24개**. 프리픽스 디스패치 테이블, 선택 상태 전이, 설정 항목 왕복·거부, `execute_selected`의 상태 분기(도움말 시드 전환·bang 힌트·미지 명령→:help·설정 모달), 드릴다운 진입/복귀를 덮었다.<br>의도적 제외: 셸 실행·클립보드·브라우저 열기·config 저장처럼 **부작용이 상태 밖으로 나가는 분기**는 단위 테스트 대상이 아니다 |
| **R2-4. cargo-deb 한글 재발 방지** | ✅ 완료 (2026-08-28) — `scripts/check-encoding.sh` + CI 잡. UTF-8 유효성(CP949 혼입)과 BOM 규칙을 본다. **`.ps1`은 BOM이 필수**(PS 5.1이 없으면 ANSI로 읽어 한글을 깨뜨림), 나머지는 금지. 도입하자마자 `assemble-config.ps1`에 BOM이 빠져 있던 것을 잡았다. 대상 파일이 0개면 "통과"가 아니라 실패(exit 2)로 본다 — 조용한 통과가 가장 위험하다 |
| **R2-5. 설정 키 4중 열거** | ✅ 대부분 해결 (2026-08-28) — `config_registry!` 매크로가 **한 줄 선언에서 get/set 양쪽을 생성**한다. 단순 키 27개(숫자·불리언·문자열)는 비대칭이 원천적으로 불가능해졌고 `get_value`/`set_value`의 중복 arm 54개가 사라졌다. 리스트 정규화·읽기 전용 의사 키 등 16개는 여전히 명시적 arm(정당함). 테스트가 레지스트리↔명시적 arm 중복까지 막는다.<br>남은 것: **UI 목록(`items.rs`)은 여전히 별도 열거** — 라벨·위젯·설명이 붙어 있어 합칠 수 없고, 왕복 테스트가 누락을 막는다 |
| **R2-6. 프리픽스 라우팅 이중화** | ✅ 부분 완료 (2026-08-27) — 두 UI가 각자 벗겨내던 프리픽스 문자열(`:emoji`\|`:e`, `!`\|`>`, `:keymap`\|`:km`)을 `kmd_core::query_prefix`의 순수 함수로 모았다. **나머지는 의도적으로 통합하지 않았다** — `transform`(데스크톱=결과 항목/TUI=상태 메시지), `prompt`·`keys`(캐시된 config vs 매번 디스크 로드), `web`(점수 부여 상이)는 실제로 갈라진 동작이다. 통합하려면 먼저 어느 쪽이 옳은지 정해야 한다 |
| **R2-7. TUI가 검색마다 config를 디스크에서 읽는다** | ✅ 완료 (2026-08-28) — `AppState.config` 캐시를 두고 `:keys`/`:keymap`/`:prompt`/`?` 핸들러가 이를 쓴다. 설정을 바꾸는 경로(설정 모달 저장, 폴더 제안 추가, 키맵 액션, 템플릿 add/remove)는 캐시를 직접 고쳐 어긋나지 않게 했다. `load_config()` 호출이 11곳 → **1곳**(시작 시 1회)<br>✅ 잔여도 해소 (2026-09-13) — 메인 루프의 `config`는 초기 셋업 구간에서만 쓰이고 이후 정본은 `AppState.config` 한 벌이다. 대신 **다른 중복이 있었다**: `AppState`가 config를 들고서도 `show_preview`·providers·prefixes 등 12개를 미러링하고 저장 시 12줄을 손으로 재대입했다(필드가 늘 때 누락되기 쉬움) → `sync_config_mirrors()` 하나로 모으고, 초기화·저장 양쪽이 같은 함수를 쓴다. "config를 고치면 전부 따라온다"를 지키는 테스트 2개 추가 |
| **R2-8. 데스크톱 settings 핸들러 비대** | ✅ 완료 (2026-09-13) — `app/settings.rs`가 662줄 중 520줄을 두 함수에 담고 있었다. `handle_settings_query` 246줄 → 23줄(목록 생성을 `settings_rows`로 분리), `handle_settings_action` 274줄 → 106줄. 핵심은 **프로바이더 4군(LLM·멀티웹·맞춤법·번역) 통합** — 라벨·접두·아이콘만 다르고 동작이 같아 `PROVIDER_GROUPS` 표 + `toggle_provider` 하나로 합쳤다(목록 생성 4벌·토글 분기 4벌 약 190줄 제거). "전부 끄면 기본값 복구" 규칙도 한 곳이 됐다. 분해 전 characterization 테스트 7개를 먼저 깔았다 |

## 3. 확장 구조 (P3 — 분리 구조 유지 결정 하에)

배경: 확장(클립보드 등)이 필요로 하는 자원(상주 상태·주입·이전 창)은 데몬에
있는데 확장은 UI에서 돈다. 상세 분석은 2026-08-08 세션 기록 참조.

| 항목 | 상태 |
|---|---|
| **R3-1. InjectPaste 능력** | ✅ E2E 검증 완료 (2026-08-08) — 데몬이 IPC로 전경 앱(TextEdit)에 Cmd+V 주입, 표식 문자열 붙여넣기 실증. 분리 구조에서 붙여넣기류 확장이 가능함이 증명됨 |
| **클립보드 P1 (docs/12 흐름 A)** | ✅ 2026-08-08 — 데몬 수집 + clip:N 즉시 붙여넣기, E2E 검증. R3-2 불필요(전경 앱에 이미 포커스). |
| **R3-2. 전경창 캡처** | ✅ 2026-08-08 — NSWorkspace 전경 PID 캡처/활성화, Launch 시점 캡처, E2E 검증(캡처→타앱→복귀 붙여넣기). docs/12 P2 |
| **클립보드 P1.1 프라이버시** | ✅ Concealed(비번) 제외 + changeCount 감시(macOS) |
| **클립보드 P3 런처 UI** | ✅ 2026-08-08 구현 — `;`/`:clip` 검색→Enter=이전 앱 붙여넣기. 라우팅·IPC 검증, GUI 상호작용만 실기기 확인 대상. docs/12 §6 |
| **R3-3. 삼중 중복 정리** | 🔶 일부 완료 (2026-09-13) — **인덱스 캐시 로드→빌드→저장** 3단 절차가 CLI(`src/cmd`)와 데스크톱(전체·quick 2벌)에 각자 적혀 있던 것을 `store::load_cached_or_build`로 모았다. 호출자별로 다른 정책(freshness 유무, 무엇을 빌드할지)만 인자로 넘기고, 데스크톱의 소요 시간 로깅은 `load_index_logged`가 공유한다. core 테스트 4개 추가.<br>남은 것: 엔진 생성 3곳(`daemon/server.rs`·`desktop/engine.rs`·`tui/app.rs`)과 프리픽스 디스패치 테이블 2벌(`desktop/search_routing.rs`↔`tui/app.rs`). 순수 함수는 R2-6에서 이미 공유했으므로 남은 건 배선인데, **어느 UI의 동작이 정본인지 정해야 통합할 수 있다**(R2-6 보류 사유와 같음) |
| **R3-4. ExtensionAction 확장** | ⬜ `Paste`/`Inject` 추가 + 순수형(클라이언트)/능력형(데몬 IPC) 경계 도입 |

## 4. 외부 대기

- **winget**: PR #413755 검증 통과, 모더레이터 머지 대기 → 머지 후 `WINGET_GITHUB_TOKEN` 등록 + 저장소 개명(`docs/07 §6` 런북)
- **Windows 창 문제**: R1-1 진단값 (사용자 Windows 머신)

## R2-6 / R3-3 결론 (2026-09-13, 설계 결정 후 실행)

두 UI의 프리픽스 핸들러를 전수 비교한 결과, **"어느 UI가 옳다"가 아니라 항목별로
갈렸다.** 백로그의 옛 근거 하나("캐시 vs 매번 디스크 로드")는 R2-7로 이미 무효였다.
좋은 쪽을 골라 core로 올리는 방식으로 처리했다.

| 쟁점 | 채택 | 이유 |
|---|---|---|
| 결과 없음 | 데스크톱 (한글 완화 + 웹 폴백) | 빈 화면을 없앤다 |
| URL 입력 | TUI (`Open <url>` 항목) | 없으면 여는 방법 자체가 없다 |
| 빈 입력 | TUI (최근 이력) | 처음 연 사람의 유일한 학습 단서 |
| config 저장 | 데스크톱 병합 + TUI 오류 노출 | 남의 변경을 안 덮으면서 실패는 알린다 |
| 무거운 작업 | 데스크톱 (워커·세대 가드) | UI 프리즈 방지 |
| 성공/실패 알림 | **새 채널** (양쪽 상태 라인) | 이벤트를 결과 목록에 끼워 넣지 않는다 |

부수 성과 — 조사에서 드러난 사용자 피해 2건을 함께 고쳤다:
`:t`가 검색 단계에서 URL을 열어 **타이핑 중 키마다 탭이 열리던 것**(이제 실행
항목 + Enter), 데스크톱 `:emoji` Enter가 **아무것도 복사하지 않던 것**.

남은 것: 엔진 생성 3곳의 배선 통합. 동작 차이가 아니라 순수 중복이라 언제든
가능하지만 이득이 작다.

## 정확성·동시성 정리 (2026-09-20, 외부 분석 REF-01~04)

앞선 "코드로 정리할 것은 소진됐다"는 결론은 **틀렸다.** 그 판단은 구조(파일·함수
크기)만 봤고, 정확성·동시성은 보지 않았다. 외부 분석이 지적한 8건을 코드로
확인한 결과 전부 사실이었고 v0.16.5로 수정했다.

| 항목 | 무엇이 문제였나 |
|---|---|
| REF-02 실행 결과 | Quick Action이 종료 코드·stderr를 버려 실패가 "(no output)"으로 보임. TUI 복사가 `let _ =`로 실패를 버리고 무조건 "Copied". 데스크톱은 실행 실패에도 창을 닫아 성공처럼 보임 |
| REF-01 설정 저장 | TUI가 저장 **전에** state 교체·dirty 해제 → 실패 시 파일·메모리·화면 3중 불일치. config 임시 파일명 고정이라 동시 저장 시 충돌 |
| REF-03 TUI 종료 | raw mode 구간의 `?` 조기 반환 시 터미널 미복구 → RAII 가드 |
| REF-04 인덱스 | 주기 리프레셔와 IPC 재생성이 조율 없이 중복 스캔. 캐시 tmp가 PID 기준이라 같은 프로세스의 두 스레드를 구분 못 함(주석엔 "프로세스별 고유") |
| 빌드 재현성 | `Cargo.lock` 미추적 + `--locked` 없음 → 같은 태그가 빌드 시점마다 다른 의존성 |

**교훈**: 자기 주석을 근거로 삼지 말 것. 인덱스 tmp는 "프로세스별 고유"라고
적어두고 정작 같은 프로세스의 경쟁을 막지 못했다.

### 후속 검토 반영 (2026-09-21, v0.16.6)

v0.16.5 재검토에서 **남은 구멍 5개**가 지적됐고 전부 사실이었다.

| 항목 | 무엇이 남아 있었나 |
|---|---|
| Windows CI 실패 | 새로 넣은 저장-실패 테스트가 "없는 디렉터리"를 썼는데 `save()`가 부모를 만들어 성공 → 테스트가 깨짐. **일반 파일을 부모로** 지정해 OS 무관하게 실패를 유도하고, "정말 실패하는가"를 전제 단언으로 박았다 |
| GUI Quick Action | `handle_shell_done`이 실행 실패·복사 실패 모두 로그만 남기고 `iced::exit()` → 종료 코드 검증을 넣어도 GUI에선 오류가 안 보였다. 성공했을 때만 닫는다 |
| 설정 변경 유실 | 임시 파일 분리는 덮어쓰기를 못 막는다. `Config::update_and_save`로 **저장 직전에 파일을 다시 읽어 변경분만 적용**. TUI 모달은 레지스트리 키를 원본과 비교해 바뀐 것만 얹는다. 폴더 제안도 저장 성공 후에만 메모리 반영 |
| 터미널 가드 위치 | `execute!` 뒤에 가드를 만들어, 그 사이 실패하면 raw mode만 켜진 채 남았다. `enable_raw_mode()` 직후로 옮기고 Drop의 복구 단계를 개별 실행으로 분리 |
| 릴리스 게이트 | 빌드 잡이 `validate-version`만 의존해 **테스트 실패에도 게시**됐다(v0.16.5 실사례). `test-release-commit` 잡을 추가해 3개 OS 테스트를 태그 커밋에서 돌리고, 모든 빌드가 이를 거치게 했다 |

**교훈 2**: 릴리스 전에 CI 결과를 확인하지 않았다. 사람이 기억하는 대신
파이프라인이 막도록 고쳤다.

남은 것(REF-05~08): UI 작업 비동기화, 실행 액션 타입화, UI 상태 분리,
IPC 연결 상한. 아래 착수 순서에 반영했다.

## 권장 착수 순서 (2026-09-20 갱신)

1. **REF-05 UI 작업 비동기화** — GUI `:f`/`?`의 디렉터리·DB 조회, TUI의 셸 실행과
   키 입력마다 도는 frecency DB 조회가 이벤트 루프를 막는다. 데스크톱 클립보드의
   세대 가드 패턴을 확장하되, "오래된 결과 폐기"와 "오래된 작업 취소"는 별개다
2. **REF-06 실행 액션 타입화** — `kmd:help:` 같은 문자열 마커를 두 UI가 각자
   재해석한다. 검색 결과 → 화면 경계에 `LauncherAction` 타입을 두는 편이 안전
3. **REF-07 데스크톱 App 상태 분리** — 검색·창 크기·포커스·부팅·설정이 한 객체에
   모여 있다. 파일을 쪼개는 게 아니라 **변경 가능 범위를 줄이는 것**이 목적
4. REF-08 IPC 연결 상한 (로컬 전용이라 급하지 않음)

## 이전 착수 순서 (2026-09-13)

코드로 정리할 수 있는 것은 대체로 소진됐다. 남은 큰 항목은 전부 **설계 결정이
먼저**라 착수 순서가 아니라 결정 순서로 적는다.

1. **R1-4 데몬 `.app` 번들화** — TCC 재부여 마찰의 근본 해법이지만 빌드·배포
   파이프라인(`scripts/deploy-local.sh`, docs/07)을 함께 고쳐야 한다. 할지 말지부터
2. **R3-4 `ExtensionAction` 확장** — `Paste`/`Inject` 추가 + 순수형(클라이언트)/
   능력형(데몬 IPC) 경계 도입. R3-3의 남은 절반과 묶어서 판단하는 게 자연스럽다
3. ~~R2-6 / R3-3 잔여~~ ✅ 위 「R2-6 / R3-3 결론」 참조 (2026-09-13 완료)
4. `src/tui/settings/items.rs`의 `items_for_tab`(254줄), `src/tui/app.rs`의
   `execute_selected`(250줄) — 다음 R2-2급 후보. 순수 데이터/분기라 분해는 쉽다
5. 보류 유지: `app/view.rs`의 `view`·`view_detail_panel`(iced 레이아웃 빌더),
   `ipc::Response::status_lines`의 표시 문구(새 소비자가 생기면 presenter 분리)

**보류 중** — 판단이 필요해 손대지 않은 것:
- `ipc::Response::status_lines`가 프로토콜 크레이트에 표시 문구를 갖고 있다
  (Codex 지적). 동작 결함은 아니고, 다음 출력 형식(JSON/영문)이나 소비자가
  생길 때 `StatusView` + presenter로 나누는 게 맞다
- 프리픽스 핸들러 중 갈라진 것들(R2-6) — 통합하려면 먼저 어느 동작이 옳은지
  정해야 한다
